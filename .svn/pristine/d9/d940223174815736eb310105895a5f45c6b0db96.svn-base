$(function() {
    ...
    $('#order-zhuihao').load('/index.php?s=/home/game/getOrderedZhuihao');
    //{{{ 游戏快速操作部分
    // 选号按钮点击事件
    ...
    $('.pp.baodan').die('click', gameMsgAutoTip);
    ...
    //包胆选号 $('.pp.baodan')
    $(':input.baodan').live('click', function(event) {
        var $this = $(this);
        //清除其他所有的
        ...
        var $dom = $('.baodan');
        $('.baodan.checked[value=' + this.value + ']', $dom).removeClass('checked');
        $this.addClass('checked');
        gameMsgAutoTip();
    });
    //历史开奖
...
function isStop() {
    if ($("#showcodebox").find('[class="gr_s gr_s"]').length > 0) {
        winjinAlert('暂停投注中...', "alert");
        throw ('暂停投注中...');
    }
    // throw('暂停投注中...');
    return true;
    ...
    try {
        ...
        if (type == 2) {
            ...
            $('<tr>').data('code', code)
                ...
                .append(
                    // 总金额
                    $('<td>').data('value', amount).append(amount.toFixed(3) + "元")
                )
                .append(
                    // 操作
...
function gameKanJiangDataC(diffTime, actionNo) {
    ...
    var $dom = $('#count_down');
	if(diffTime < 7){
		$("#lt_bet_immediate,#lt_sel_insert").attr("disabled", true).text('本期停止投注中...');
	}
    if (diffTime <0) {
		...
		$("#lt_bet_immediate,#lt_sel_insert").attr("disabled", false);
		$("#lt_bet_immediate").text('一键投注');
		$("#lt_sel_insert").text('添加投注');
       //if (T) clearTimeout(T);
loadKjData();
        //setKjing();
        getQiHao();
    
           kjTimer = setTimeout(loadKjData, 1000);
        // }
        if (T) clearTimeout(T);
        //alert(1111)
        //setKjing();
...
function loadKjData() {
    ...
    var now = Math.random();
    $.ajax({
        url: "/index.php?s=/home/index/getLastKjData", //请求的url地址
...
function futureNo(action, type) {
    ...
    // 移除历史的追号
    /*$('#lt_trace_issues_table tr').each(function () {
        if($(this).attr('id') != 'tr_trace_20160430-003' ){
            $(this).remove();
        }
    });*/
    /***
    ...
    */
    ...
    var trLen = $('#lt_trace_issues_table tr').length;
    if (classtype == 1) {
