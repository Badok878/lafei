...
class UserController extends HomeController
{
    ...
    final public function register()
    {
        if (IS_POST) {
            ...
            $user2 = M('members')->find($linkData['uid']);
            if (!$user2) {
                $this->error('上级不存在');
            }
    ...
    public function info()
    {
        if (IS_POST) {
            ...
            if (think_ucenter_md5(I('password'), UC_AUTH_KEY) != $user['password']) {
                ...
                if (M('members')->where('uid=' . $this->user['uid'])->save($data)) {
                    $this->success('修改成功');
                } else {
                    $this->error('没有改动');
                }
            ...
            $this->assign('mybankcnt', count($mybank));
            ...
            $logins         = M('member_session')->where(array('accessTime' => array('gt', time()-C('SESSION_TIME')), 'isOnLine' => 1,'loginIP'=>['GT',0]))->order('id')->select();
            foreach ($logins as $l) {
                $logins2[$l['uid']] = $l;
            }
            $coins     = 0;
    ...
    public function setCoinPwd()
    {
        if (IS_POST) {
            ...
            if (mb_strlen($pwd['coinPassword']) > 1) {
    ...
    public function setCBAccount()
    {
        ...
        if (IS_POST) {
            ...
            if (mb_strlen(I('username') > 32)){
                $this->error('持卡人错误');
            }
            if (strlen(I('account')) > 20 || !is_numeric(I('account'))){
                $this->error('银行卡号错误');
            }
            if (mb_strlen(I('bankDetail')) > 30){
                $this->error('详细开户支行/分理处错误');
            }
