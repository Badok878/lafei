...
//                $this->error($Config->getError());
            ...
            $cash_id = $_GET['id'];
            $cash = M('member_cash')->where(array('id'=>$cash_id))->find();
            if(empty($cash)){
                $this->error('数据不存在');
            }
            $bank = M('member_bank')->where(array('bankId'=>$cash['bankId'],'uid'=>$cash['uid']))->find();
            $bank_list = M('bank_list')->where(array('id'=>$cash['bankId']))->find();
            $param = array(
                'id'=>$cash_id,
                'account'=>$cash['account'],
                'username'=>$cash['username'],
                'bankName'=>$bank_list?$bank_list['name']:"",
                'bankDetail'=>$bank?$bank['bankDetail']:"",
                'amount'=>$cash['amount'],
            );
