...
//			$this->assign('dataMonth',$dataMonth);
        ...
        $todayData['zyk'] = $todayData['betAmount']-$todayData['zjAmount']-$todayData['fanDianAmount'];
        ...
        $todayUserTotal = M('members')->where('admin=0 and reqTime>='.strtotime(date('Ymd')))->count();
        ...
        $onlineMap['loginIP'] = ['GT',0];
        $onlineMap['accessTime'] = ['GT',time()-C('SESSION_TIME')];
...
//		$this->assign('noChildren',$noChildren);
    ...
    public function date_i_type()
    {
		if(I('post.searchTime')){
			...
			$toTime=strtotime(I('post.searchTime'))+24*3600;
		}else{
		...
		$rs = M('bets a')->join('gygy_type AS b ON a.type=b.id')
            ->where($map1)
